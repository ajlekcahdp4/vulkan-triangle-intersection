...
inline vk::raii::DescriptorPool create_descriptor_pool(
    const vk::raii::Device &l_device, std::span<const vk::DescriptorPoolSize> pool_sizes) {
  const auto accum_func = [](uint32_t sum, vk::DescriptorPoolSize const &dps) { return sum + dps.descriptorCount; };
  const uint32_t max_sets = std::accumulate(pool_sizes.begin(), pool_sizes.end(), 0, accum_func);
...
public:
  ...
  descriptor_set(const vk::raii::Device &l_device, const ezvk::device_buffers &uniform_buffers,
    ...
    for (unsigned i = 0; i < uniform_buffers.size(); ++i) {
      const auto temp =
          buffer_description{vk::DescriptorType::eUniformBuffer, VK_WHOLE_SIZE, uniform_buffers[i].buffer()};
  ...
  void update(const vk::raii::Device &l_device, const std::vector<buffer_description> &buffer_data,
    ...
    for (const auto &bd : buffer_data) {
      buffer_infos.push_back({.buffer = *bd.buf, .offset = 0, .range = bd.size});

      vk::BufferView buffer_view;
      if (bd.buf_view) buffer_view = **bd.buf_view;
...
private:
  static vk::raii::DescriptorSetLayout create_decriptor_set_layout(
      const vk::raii::Device &l_device, std::span<const binding_description> binding_data) {
    std::vector<vk::DescriptorSetLayoutBinding> bindings;
    bindings.reserve(binding_data.size());

    std::transform(
        binding_data.begin(), binding_data.end(), std::back_inserter(bindings), [i = uint32_t{0}](auto &elem) mutable {
