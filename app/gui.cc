...
imgui_resources::imgui_resources(const applicaton_platform &plat, const vk::raii::Device &l_device,
  ...
  m_imgui_render_pass = ezvk::render_pass(l_device, subpass, attachments, imgui_subpass_dependency);
  ...
  ImGui_ImplVulkan_InitInfo info = {.Instance = *plat.instance(),
      .PhysicalDevice = *plat.p_device(),
      .Device = *l_device,
      .QueueFamily = graphics.family_index(),
...
void app_gui::draw() {
  ...
  if (ImGui::CollapsingHeader("Movement", ImGuiTreeNodeFlags_DefaultOpen)) {
    ...
    ImGui::DragFloat("Linear velocity (regular)", &params.linear_velocity_reg, 1.0f);
    ImGui::DragFloat("Linear velocity (mod)", &params.linear_velocity_mod, 10.0f);
  ...
  if (ImGui::CollapsingHeader("Rendering", ImGuiTreeNodeFlags_DefaultOpen)) {
    ...
    ImGui::DragFloat("Fov", &params.fov, 0.1f, 45.0f, 175.0f, "%.3f", ImGuiSliderFlags_AlwaysClamp);
    ...
    ImGui::ColorEdit4("Intersecting", params.colors[config::intersect_index].data());
  ...
  if (ImGui::CollapsingHeader("Color", ImGuiTreeNodeFlags_DefaultOpen)) {
    ...
    ImGui::DragFloat(
        "Ambient Strength", &params.ambient_strength, 0.001f, 0.0f, 1.0f, "%.3f", ImGuiSliderFlags_AlwaysClamp);
    ...
    ImGui::DragFloat("Yaw", &params.light_dir_yaw, 0.1f, 0.0f, 360.0f, "%.3f", ImGuiSliderFlags_AlwaysClamp);
    ImGui::SameLine();
    ImGui::DragFloat("Pitch", &params.light_dir_pitch, 0.1f, 0.0f, 360.0f, "%.3f", ImGuiSliderFlags_AlwaysClamp);

    params.light_dir = glm::eulerAngleYX(glm::radians(params.light_dir_yaw), glm::radians(params.light_dir_pitch)) *
        glm::vec4{0, 0, 1, 0};

    auto light_dir = params.light_dir;
